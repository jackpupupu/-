...
public class BaoBiaoShengCheng {
    ...
    public static class XueShengBaoBiao implements Serializable {
        ...
        // 设置科目名称和成绩
        public void setKeMuChengJi(String keMu, double chengJi, int index) {
            if (index >= 0 && index < keMuMing.length) {
                keMuMing[index] = keMu;
                keMuChengJi[index] = chengJi;
            }
        }
        ...
        // 设置科目班级平均分
        public void setKeMuBanJiPingJun(double pingJunFen, int index) {
            if (index >= 0 && index < keMuBanJiPingJun.length) {
                keMuBanJiPingJun[index] = pingJunFen;
            }
        }
